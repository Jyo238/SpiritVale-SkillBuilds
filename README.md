# SpiritVale Skill Builds（Build 切換）

打王要補血、練功要輸出，換一次流派就得重點一次技能、再把整排快捷鍵重擺一遍——
洗完點快捷列還會**整排消失**。

本插件讓你把 **技能點 ＋ 能力點 ＋ 裝備／神器／魔導書 ＋ 快捷列**（想要的話再加上
**時裝／坐騎／寵物 ＋ 外觀**）存成一個 Build，之後**點兩下全部換回來**。

### 一鍵之前 → 一鍵之後

在 Waybinder 重置之後的狀態：快捷列整排空白、裝備全被卸下、385 點能力點待重配。

| 還原前 | 還原後 |
|---|---|
| ![還原前的快捷列](docs/before-hotbar.png) | ![還原後的快捷列](docs/after-hotbar.png) |
| ![還原前的角色](docs/before-character.png) | ![還原後的角色](docs/after-character.png) |

中間你只做了一件事：**在 Build 按鈕上點兩下**。

## 給玩家：三步安裝

1. 到 [Releases](https://github.com/Jyo238/SpiritVale-SkillBuilds/releases) 下載最新的一鍵安裝包 zip
2. 完整解壓縮後，關閉遊戲，雙擊 **`一鍵安裝.bat`**（自動偵測遊戲位置；沒裝過 BepInEx 會自動下載安裝）
3. 開遊戲。**首次啟動會多花 1~3 分鐘初始化屬正常**（已裝繁中翻譯包／其他 BepInEx Mod 者無此等待）

與「繁中翻譯包」「紫星販賣保護」「交易所比價」「詞條快篩 HUD」完全相容可共存。
出問題請雙擊 `產生診斷報告.bat`，把桌面上的報告貼到回報處。
**更新方式**：下載新版 zip 後重跑 `一鍵安裝.bat` 即可（會自動偵測並覆蓋舊版）。

## 操作

打開技能視窗，右上角會多一排 Build 按鈕。

| 操作 | 行為 |
|---|---|
| **Shift + 左鍵** | 把目前配置**整組**存進這一組（覆寫既有的要按兩次確認——它會用你身上的一切蓋掉那組） |
| **左鍵點兩次** | 還原這一組（第一次是確認，3 秒內再點一次） |
| **右鍵** | 編輯：改名、勾選要還原哪幾類、只更新時裝／外觀、刪除 |
| **＋** | 再加一組（最多 12 組） |
| **Ctrl + F1~F6** | 熱鍵還原（不用開技能視窗） |
| **Ctrl+Shift + F1~F6** | 熱鍵儲存 |

還原順序：**套用點法 → 配能力點 → 換裝 → 時裝／外觀 → 綁快捷列**，畫面顯示每一步進度。
裝備必須排在快捷列之前——裝備賦予的技能要先回到身上，那些格子才綁得上；
武器外觀要對手上的武器，所以時裝排在換裝之後。

## 每組可以只換一部分

![編輯面板](docs/ui-edit-panel.png)

右鍵開編輯面板，九個勾選各自開關：技能點／快捷列／能力點／裝備／神器／魔導書／
**時裝（含坐騎／寵物）／外觀（長相）**／卸下快照沒有的裝備／時裝欄。

**儲存時永遠全部記下來，勾選只決定還原時要動哪些**——隨時改主意都不用重存快照。
善用這點可以做出「只勾裝備」的**純換裝組**（打王裝／練功裝／打錢裝），完全不碰點法與能力點；
或「只勾時裝＋外觀」的**純造型組**。按鈕上顯示 `*` 即代表該組是部分還原。

- **時裝**＝衣櫃裡套用的一切：外裝、武器外觀、坐騎、寵物、光環／拖尾、稱號、徽章…
  走遊戲衣櫃「套用」同一條路，只能套用衣櫃裡已擁有的；武器外觀跟手上武器不相容時跳過並列出。
- **外觀**＝人物長相十項（膚色／髮型／髮色／眉／鬍／嘴／眼／眼色／耳／瞳），
  走衣櫃「外觀」頁籤／造型師 NPC 同一條路，免費、無次數限制。
- 這兩項**新 Build 預設不勾**（設定檔可改）：打扮通常是「人」的不是「流派」的——預設勾上的話，
  存完 Build 之後換的坐騎、改的髮型會在還原時被靜靜換回去。想每組各一套打扮的人，右鍵那組打勾就好。
- **打勾只是開關，資料要那一組自己存過才有。** 升級前存的 Build 沒有時裝／外觀資料（還原時會明講）；
  補資料不用整組重存——右鍵那組 →「把目前的時裝／外觀存進這組（其他不動）」。

## 防手滑

- **Shift+左鍵是「整組覆寫」**：把你目前身上的技能／裝備／打扮全部蓋掉那組。所以覆寫既有 Build 要按兩次確認。
  典型事故：剛還原完 A、穿好衣服想幫 B 存個造型就 Shift+點 B → B 的點法變成 A 的。只更新造型請用右鍵面板那顆按鈕。
- 被覆寫／刪除的舊 Build 自動備份到同資料夾的 `local.spiritvale.skillbuilds.presets.backup.json`
  （每角色最近 30 筆，含時間與原因）。要救回：關遊戲，把那筆的 `Preset` 貼回 `presets.json` 對應位置。

## Build 存哪裡 / 換電腦

Build 依**角色**分開存（鍵是伺服器端的角色 id，同帳號不同角色互不干擾），檔案預設在
`<遊戲>\BepInEx\config\local.spiritvale.skillbuilds.presets.json`。

換電腦把這個檔案複製過去即可；或在設定檔填「存檔資料夾」指向雲端同步資料夾
（OneDrive／Google 雲端硬碟等），兩台電腦自動共用（別兩台同時開遊戲改，會打架）。

## 技術

- BepInEx 6（IL2CPP）+ HarmonyX，與 [SpiritVale-SellFavorite](https://github.com/Jyo238/SpiritVale-SellFavorite)、
  [SpiritVale-MarketPrice](https://github.com/Jyo238/SpiritVale-MarketPrice) 同一套工具鏈。
- **一行遊戲邏輯都沒有重造**：全程呼叫遊戲自己的客戶端入口——
  `PlayerSave.ApplySkills` / `ApplyAttributes` / `AssignSkill` /
  `ApplyEquip` / `ApplyArtifact` / `ApplyGrimoire` / `ApplyCosmetic` / `ApplyAppearance`，
  與你在 UI 手動操作走的是同一條路，伺服器端驗證完全不變。
- **狀態機用「輪詢謂詞」推進**，不 patch 帶編譯雜湊的 RPC 方法：每一步伺服器回應都會把本地
  `CharacterData` 整包換新，所以直接對資料驗收（技能等級全對、能力點六項全對、該格裝備 uid 相符）
  才前進——不是送出就當成功，也不會因為遊戲改版動到方法雜湊就失效。
- **零 delegate、零類別注入**：UI 全部手建 `Image`/`TMP`/`TMP_InputField`，
  互動靠每幀 raycast 與按鍵輪詢（本遊戲的 Il2CppInterop 注入 hook 有崩潰前科，見 SellFavorite 的 README）。
- 逆向得到的關鍵事實：
  - **本插件不呼叫任何「重置」**：`ResetSkills` / `ResetAttributes` 是 Waybinder NPC 專屬入口。
    技能直接送最終點法（`ApplySkills` 收完整終態、可增可減，等同玩家右鍵退點後按套用）——
    隨地可用，而且是原子操作：伺服器要嘛整包接受、要嘛整包拒絕，失敗時原本的點法不會被動到。
    能力點只送**正差額**；需要調降時跳過能力點並提示玩家自己去 Waybinder 重置
    （伺服器端其實沒擋負差額，但那是遊戲介面做不到的事，刻意不做）
  - `ApplySkills` 收的是**完整終態清單**；`ApplyAttributes` 收的是**差額**
  - 換裝有伺服器閘門 `CheckCanChangeGear` → **戰鬥中禁止**，所以還原前先擋
  - **飾品左右欄由遊戲決定**（`ApplyEquip` 只收物品不收欄位）。伺服器端規則：
    **唯一（Unique）飾品一律進左欄**、左欄原本的擠回背包；一般飾品左空進左、左滿右空進右、
    **兩欄都滿一律換左欄**（右欄永遠不會被直接換掉）；且已穿在身上的物品再送 `ApplyEquip` 會被靜默忽略。
    → 插件依這套規則**先模擬再送**（卸不要的 → 唯一件先穿 → 一般件補空欄，兩欄都滿先讓出不要的那欄），
    飾品以「兩件都在身上」為到位、不計較左右；換裝跑完仍有總驗收＋補裝重試兜底
- 防呆：啟動時以反射驗證三個 RPC 簽名，不符就停用還原並提示等待更新；
  發送碼全關在 `NoInlining` 方法內隔離；缺件／需求不符明確列出而非靜默略過。

## 建置

需要 .NET SDK 8+、遊戲已安裝 BepInEx 6.0.0-be.785（IL2CPP win-x64）且啟動過一次（產生 `BepInEx\interop\`）。

```powershell
dotnet build src\SpiritValeSkillBuilds.csproj -c Release
# 遊戲不在預設路徑時：-p:GameDir=<遊戲目錄>
Copy-Item src\bin\Release\net6.0\*.dll <遊戲>\BepInEx\plugins\SpiritValeSkillBuilds\
```

## 風險聲明（使用前請讀完）

**線上遊戲使用第三方工具都有風險，請自行斟酌。**

這是**第三方 Mod，不是官方功能**。官方沒有背書，也隨時可能改變立場。
以下把事實攤開，讓你自己判斷划不划算。

### 它做什麼

把你的技能點、能力點、裝備／神器／魔導書、快捷列存起來，之後一次還原回去。
**所有動作都走遊戲自己的客戶端入口**（`ApplySkills` / `ApplyAttributes` / `AssignSkill` /
`ApplyEquip` / `ApplyArtifact` / `ApplyGrimoire`），跟你在介面上手動操作發出的請求完全相同，
伺服器端驗證一項都沒有被跳過。差別只在於由程式代勞，不必一格一格點。

### 它不做什麼

- **不呼叫任何「重置」**——重置技能／能力點是 Waybinder NPC 專屬功能，那一步請你自己去按
- 不修改任何數值、不偽造封包、不自動化戰鬥、不繞過任何遊戲限制、不碰別人的資料
- **能力點只加不減**：遊戲介面本來就只有 `+`。需要調降時插件會跳過能力點並提示你去 Waybinder，
  而不是去做「介面上做不到的事」（伺服器端其實沒擋，是我們刻意不做）
- 換裝遵守伺服器的戰鬥中禁止換裝閘門（`CheckCanChangeGear`），不嘗試繞過

### 官方目前的立場（以及它的三個但書）

2026/8/5 有玩家在 Discord 詢問官方對漢化包與詞條工具的立場，SPR 團隊回覆大意是
**「目前允許使用非官方工具，但官方不負責，帳號問題與資料遺失由玩家自行承擔」**。

這是好消息，但請連同三點一起看：

1. 那是**即時問答，不是正式公告**，立場隨時可能改變
2. 當時被問到的是**「不對伺服器多送東西」的顯示類工具**（漢化包、詞條 HUD）。
   本插件會實際送出換點與換裝的請求，**性質不完全相同，不能直接套用那個答覆**
3. 官方明講**「資料遺失自負」**——所以本插件刻意讓每一步都可獨立失敗而不破壞現況

### 風險，請自己評估

- 修改客戶端**有可能被認定違反服務條款**，最壞的情況是帳號受影響，**後果自負**
- 遊戲改版可能讓它失效，甚至造成非預期行為——**改版後請先停用，等更新確認過再開**
- 重置技能／能力點的遊戲內成本（費用、道具）照算，本插件不會、也不能幫你省
- 本遊戲目前沒有反作弊程式，但**「現在沒有」不等於「以後不會有」**

### 幾個務實建議

- 先在自己不心疼的角色上試一輪，確認流程符合預期再用在主力角色
- 官方有任何相關公告就立刻停用
- 不必在公開頻道大肆宣傳（對你我都好）

技術面補充：還原技能點是**原子操作**——伺服器要嘛整包接受、要嘛整包拒絕，
失敗時你原本的點法完全不會被動到。這是刻意選擇的設計，讓最常用的那一步即使出錯也不會讓你更糟。

**覺得風險不划算就不要用，這完全合理。** 這份聲明的目的是讓你有足夠資訊做決定，不是說服你安裝。

MIT 授權。歡迎回報問題與改進建議。
